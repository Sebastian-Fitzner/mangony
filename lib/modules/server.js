...
class Server {
	...
	initialize() {
		...
		this.ext = this.options.useExt ? this.options.ext : '';
	...
	bindEvents() {
		...
		events.registerEvent(events.EVENTS.route.register, fnRegisterRoute);
	}
	...
	registerRoute(obj) {
		...
		console.info(chalk.green('Route ' + chalk.cyan(obj.route + this.ext) + ' is registered!'));

		this.app.get('/' + obj.route + this.ext, (req, res) => {
