	...
	deleteFromCache(obj) {
		...
		if (obj.type === 'data') {
			...
			if (this.collections.length && data.parsed) {
				...
				Promise.map(collections, (name) => {
					let cIdx = this.cache.collections[name][obj.data.parsed.data[name]].files.indexOf(obj.id);

					this.cache.collections[name][obj.data.parsed.data[name]].files.splice(cIdx, 1);
				});
			}
		}
