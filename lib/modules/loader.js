 ...
 * @param {String} obj.srcPath - Path to file
 * @param {String} obj.typePath - Path for calculation
 * @return {Object}
 */
function readFile(obj) {
	let file = Helpers.getFilename(obj.srcPath);
	let ext = path.extname(obj.srcPath);

	return fsx.readFileAsync(obj.srcPath, 'utf8')
		.then((data) => {
			return parseData({
				type: ext,
				data: data
		...
		.then((data) => {
			return {
				id: obj.createDeepIds === true ? Helpers.cleanId({
					srcPath: obj.srcPath,
					typePath: obj.typePath,
					pathDelimiter: obj.pathDelimiter,
					file: file
				}) : file,
				assets: Helpers.assetsPath(obj.dest + Helpers.buildPath({
						srcPath: obj.srcPath,
						typePath: obj.typePath
					}), obj.dest + obj.assets),
				ext: ext,
				basename: path.basename(obj.srcPath),
				filename: file,
				dirname: path.dirname(obj.srcPath),
				destDir: obj.dest,
				destSubDir: Helpers.buildPath({
					srcPath: obj.srcPath,
					typePath: obj.typePath
				}),
				destFile: obj.flatten ? file + obj.ext : Helpers.buildId({
					srcPath: obj.srcPath,
					typePath: obj.typePath,
					file: file
				}) + obj.ext,
