...
class JSXTemplater extends Templater {
    ...
    initialize() {
        const fileLoaders = Object.keys(this.context.data.types).filter(key => key !== 'data').map(type => {
            return loader.getFiles(`${process.cwd()}/${this.context.options.cwd}/${this.context.data.types[type].files}`);
        })

        Promise.all(fileLoaders).then(files => {
            const cleanedFiles = files
                .flat(Object.keys(this.context.data.types).length)
    ...
    bindEvents() {
        ...
        events.registerEvent(this.context.evtNamespace + this.context.events.helpers.add, createRendering('helpers'));
        events.registerEvent(this.context.evtNamespace + this.context.events.helpers.changed, createRendering('helpers'));
    }
    ...
    renderTemplate({pageContent, page, data}) {
        ...
        const withLayout = data.layout ? maybeWithLayout.default : (cmp) => (data) => cmp(data);
