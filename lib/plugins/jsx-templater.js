...
class JSXTemplater extends Templater {
    ...
    renderTemplate({pageContent, page, data}) {
        ...
        const compiledPage = requireUncached(`${process.cwd()}/${this.context.options.cwd}/.mangony/pages/${page.filename}`);
        const withLayout = data.layout ?
            requireUncached(`${process.cwd()}/${this.context.options.cwd}/.mangony/layouts/${data.__layouts[data.layout].filename}.js`).default :
            (cmp) => (data) => cmp(data);
    ...
    bundleFile({file, type}) {
        ...
        const outfile = `${process.cwd()}/${this.context.options.cwd}/.mangony/${type}/${file.filename}.js`;
