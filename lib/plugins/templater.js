...
class Templater {
	...
	renderOne(obj) {
		...
		if (this.options.YFMContextData && globalData[pageData.contextData]) {
			data = Helpers.extend(data, obj.cache[pageData.contextData]);
		}

		if (this.options.YFMLayout && globalData.__layouts) {
			if (!globalData.__layouts[pageData.layout]) {
				return console.error('No layout found!');
			}

			let layoutFile = globalData.__layouts[pageData.layout].parsed.content;

			page = layoutFile.replace('{{{yield}}}', pageContent);
		}
