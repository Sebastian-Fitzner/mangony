...
class Templater {
	...
	renderOne(obj) {
		...
		this.options.debug && console.info(chalk.green(`------- Starting to compile ${chalk.cyan(destDir + destFile)} ... -------`));


		template = this.engine.compile(page);
		content = pageSrcExt === '.md' ? md.render(template(data)) : template(data);

		this.options.debug && console.info(chalk.green(`------- Finished compiling of ${chalk.cyan(destDir + destFile)}! ------- \n`));
