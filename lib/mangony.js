...
class Mangony {
	constructor(opts) {
		this.options = {
			allow: { // Allow further options
				YFMLayout: false, // Allow layout definition in YAML front matter
				YFMContextData: false // Allow context data in YAML front matter
			},
	...
	initialize() {
		...
		this.server = this.options.devServer.start ? new Server({
				evtNamespace: this.evtNamespace,
				bs: this.options.devServer.bs,
				bsEnabled: this.options.devServer.bsEnabled,
				bsOptions: this.options.devServer.bsOptions,
				data: this.data,
				dest: this.dest,
				injectScript: this.options.devServer.injectScript,
				express: this.options.devServer.express,
				ext: this.options.ext,
				port: this.options.devServer.port,
				templater: this.templater,
				useExt: this.options.devServer.useExt,
				usePort: this.options.devServer.usePort,
				useAssetsDir: this.options.devServer.useAssetsDir
			}) : null;
		this.timeLogs = {
	...
	bindEvents() {
		...
		let renderPage = (function (obj) {
			this.loader.readFile(obj.file)
				...
				.then((fileData) => {
					...
					if (obj.evt === 'add') {
						this.events.emitEvent(this.evtNamespace + this.events.EVENTS.route.register, {
							id: fileData.id,
		...
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.pages.add, renderPage);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.pages.changed, renderPage);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.pages.delete, deleteFromCache);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.data.add, renderPagesData);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.data.changed, renderPagesData);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.data.delete, deleteFromCache);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.partials.add, renderPagesPartials);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.partials.changed, renderPagesPartials);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.partials.delete, deleteFromCache);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.layouts.add, renderPagesLyt);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.layouts.changed, renderPagesLyt);
		this.events.registerEvent(this.evtNamespace + this.events.EVENTS.layouts.delete, deleteFromCache);
	}

	reloader() {
		this.events.emitEvent(this.evtNamespace + this.events.EVENTS.cache.updated);
	}
	...
	getAllFiles() {
		...
		let helpersFiles = this.getFiles(this.options.helpers.map((helperPath) => path.resolve(this.cwd, helperPath)))
			.then((helpers) => {
				this.helpersFiles = helpers;
				return helpers;
			})
			.catch(e => console.error(chalk.red('Mangony Error :: Helpers could not be registered:\n', e)));
