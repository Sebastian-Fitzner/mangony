...
class Mangony {
	...
	initialize() {
		...
		this.templater = new Templater({
			allow: {
				YFMLayout: this.options.YFMLayout,
				YFMContextData: this.options.YFMContextData
			},
	...
	render() {
		if (!this.templater) {
			throw new Error('Mangony :: You have to use a templater plugin!');
		}
		...
		return this.prepareData()
			...
			.then(() => {
				...
				if (this.server && this.server.startServer) {
			...
			.then(() => {
				if (!this.server) this.postRenderPages();
				this.postRenderTime();
			})
			.catch((err) => {
